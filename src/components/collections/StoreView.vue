                  ...
                  <li v-for="(store, id) in availableStores" :key="id" class="store-list">
                      <div class="store-wrapper">
                        ...
                        <div class="container-fluid">
                          <p>{{ store.name }}</p>
                          <span>{{ releaseTime(collectionstores[id].date) }}</span>
                        </div>
                        <a :href="collectionstores[id].affiliateLink" target="_blank">
...
export default {
  ...
  created () {
    // DEN SKAL VÆRE NEDENUNDER måske: db.ref('collections/' + this.$route.params.id + '/stores')
    this.$bindAsArray('collectionStores', db.ref('collections/').orderByChild('siteTag').equalTo(this.$route.params.id))
    this.$bindAsObject('stores', db.ref('stores'))
    // this.arrayCollectionStores = Object.values(this.collection.stores)
  },
  props: {
    collection: {
      type: Object,
  ...
  computed: {
    availableStores () {
      ...
      var arrayStores = Object.values(this.collectionStores[0].stores).map(s => this.stores[s.storeId])
      return arrayStores
    }
  }
}
