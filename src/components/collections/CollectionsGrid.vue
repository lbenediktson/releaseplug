    ...
    <router-link :to="{ name: 'collection', params: { id: collectionId }}">
      <div class="portfolio-box">
        <div :style="'background-image: url(' + collection.imgUrl + ');'" class="portfolio-box-img">
          <!-- <p class="collection-date">{{ dateFormatter(collection.date) }}</p> -->
          <p class="productDate">{{ collection.date | moment("MMMM D") }}</p>
        </div>
        <div class="gallary-title text-center">
          <h6 class="productTitle">
            <!-- <router-link :to="{ name: 'collections', params: { id: collectionId }}"> -->
            {{ collection.title }}
            <!-- </router-link> -->
          </h6>
          <span class="productPrice">{{ collection.price }}<span v-if="collection.price != 'Unknown'">€</span></span>
        </div>
      </div>
      <!-- <router-link :to="{name: 'collections', params: { id: collectionId }}"> -->
    </router-link>
