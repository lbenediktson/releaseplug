    ...
    <router-link :to="{ name: 'sneaker', params: { id: sneakerId }}">
      <div class="portfolio-box">
        <div :style="'background-image: url(' + sneaker.imgUrl + ');'" class="portfolio-box-img">
          <!-- <p class="sneaker-date">{{ dateFormatter(sneaker.date) }}</p> -->
          <p class="productDate">{{ sneaker.date | moment("D. MMM") }}</p>
        </div>
        <div class="gallary-title text-center">
          ...
          <h6 class="productPrice">{{ sneaker.price }}<span class="sneakerCurrency" v-if="sneaker.price != 'Unknown'">€</span></h6>
        </div>
      </div>
      <!-- <router-link :to="{name: 'sneaker', params: { id: sneakerId }}"> -->
    </router-link>
...
export default {
  ...
  methods: {
    dateFormatter (dato) {
      var month = dato.substring(5, 7)
      var monthDay = dato.substring(8, 10)
      switch (month) {
        case '01':
          return monthDay + '. jan'
...
/* div.portfolio-box:hover span, a:hover {
  color: #ff5757;
...
.portfolio-box > .gallary-title > h6,
.portfolio-box > .gallary-title > span.sneakerPrice,
.portfolio-box > .gallary-title > span.sneakerPrice > span {
